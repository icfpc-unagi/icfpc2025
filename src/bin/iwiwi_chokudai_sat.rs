...
use icfpc2025::{
    judge::Guess,
    solve_no_marks::{self, Cnf, SATSolver},
    *,
};
...
fn main() {
    ...
    // 解けたらうれしいな
    // iwiwiwiwiwi ここからだけ書き換えます！！！！！vvvvvvvvvvvvvvvv
    // assert_eq!(cnf.sat.solve(), Some(true));

    let solvers: Vec<SATSolver> = (0..25)
        .map(|seed| SATSolver {
    ...
    let solution = solve_no_marks::launch_portfolio(dimacs_path, &solvers);
