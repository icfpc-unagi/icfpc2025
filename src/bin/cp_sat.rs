...
use cp_sat::proto::{self, CpSolverStatus};

struct Cnf {
    model: CpModelBuilder,
    vars: Vec<BoolVar>,
    response: Option<proto::CpSolverResponse>,
    buf: Vec<i32>,
}
...
impl Cnf {
    ...
    fn var(&mut self) -> i32 {
        ...
        self.vars.push(v);
        (self.vars.len() - 1) as i32
    }
    #[inline]
    fn lit(&self, x: i32) -> BoolVar {
        if x > 0 {
    ...
    fn clause<I: IntoIterator<Item = i32>>(&mut self, lits: I) {
        let vars = lits.into_iter().map(|lit| self.lit(lit));
        self.model.add_or(vars);
    }
    #[inline]
    fn choose_one(&mut self, xs: &[i32]) {
        let vars = xs.iter().map(|&v| self.vars[v as usize]);
        self.model.add_exactly_one(vars);
    }
    #[inline]
    fn at_most_one(&mut self, xs: &[i32]) {
        let vars = xs.iter().map(|&v| self.vars[v as usize]);
        self.model.add_at_most_one(vars);
    }
    #[inline]
    fn solve(&mut self) -> CpSolverStatus {
        let resp = self.model.solve();
    ...
    fn value(&self, var: i32) -> bool {
        self.vars[var as usize].solution_value(self.response.as_ref().unwrap())
    }
}
...
fn extract_guess(
    ...
    for u in 0..n {
        for e in 0..6 {
            ...
            for f in 0..6 {
                if cnf.value(edges.M[u][v_sel][e][f]) {
                    f_sel = f;
                    break;
                }
            }
...
fn solve(judge: &mut dyn icfpc2025::judge::Judge) {
    ...
    // 5) Solve
    assert_eq!(cnf.solve(), CpSolverStatus::Optimal);
