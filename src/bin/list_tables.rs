...
fn main() -> Result<()> {
    ...
    let rows = icfpc2025::sql::select(query, (db_name,))?;

    for row in rows {
        let table_name: String = row.at(0)?;
        println!("\nTable: {}", table_name);
        ...
        let schema_rows = icfpc2025::sql::select(schema_query, (db_name, &table_name))?;

        for schema_row in schema_rows {
            let column_name: String = schema_row.at(0)?;
            let column_type: String = schema_row.at(1)?;
            let is_nullable: String = schema_row.at(2)?;
            let column_key: String = schema_row.at(3)?;
            let column_default: Option<String> = schema_row.at_option(4)?;
            let extra: String = schema_row.at(5)?;
